const input = `8 * 6 * 6 * 4 * (8 + 5 * 2 + 5 + 6 + 3) * 4
...
const doComputation = (line) => {
    let str = line;
    let hasSymbol = str.match(/\D/g).length > 0;
    while (hasSymbol) {
        if (str.match(/\(/g)) {
            const opening = str.lastIndexOf("(");
            const closing = str.indexOf(")", opening);

            const sub = str.substring(opening, closing + 1);
            const between = str.substring(opening + 1, closing);

            str = str.replace(sub, doComputation(between));
        } else {
            var match = str.match(/(\d+[\*\+]\d+)/);
            while (match) {
                var x = match[0].match(/(?<left>\d+)(?<symbol>[\*\+])(?<right>\d+)/).groups;
                let res = ''
                switch (x.symbol) {
                    case '*':
                        res = parseInt(x.left) * parseInt(x.right)
                        break;
                    case '+':
                        res = parseInt(x.left) + parseInt(x.right)
                        break;
        ...
        hasSymbol = str.match(/\D/g) && str.match(/\D/g).length > 0;
    }
    return str;
}
...
const parse = dataSet => {
    ...
    lines.forEach(l => {
        results.push(doComputation(l));
    });

    return results.reduce((acc, curr) => acc + parseInt(curr), 0)
}
