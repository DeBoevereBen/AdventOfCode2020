...
const parse = () => {
    ...
    const questions = results.map(x => x.split("\n"));
    ...
    questions.forEach(q => {
        const numberOfPersons = q.length;
        console.log(numberOfPersons, q)

        var letters = {};

        q.forEach(l => {
            l.split("").forEach(a => {
                letters[a] = letters[a] ? letters[a]+1 : 1
            });
        })
        
        result += Object.values(letters).filter(l => l == numberOfPersons).length;
    });
