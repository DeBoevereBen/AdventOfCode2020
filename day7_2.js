const input = `dim red bags contain 2 bright gold bags, 5 striped fuchsia bags.
...
const add = (bag) => {
  ...
  return bag.reduce((prev, curr)=> {
    return prev + (add(curr.bag) * curr.amount) + curr.amount;
  }, 0)
};
...
const parse = () => {
  ...
  lines.forEach((l) => {
    const match = l.match(
      /^(?<headBag>\w+\s*\w+)\s*bags contain (?<content>[^.]*)/
    );
    bags[`${match.groups.headBag}`] = match.groups.content
      .split(",")
      .map((x) => x.trim())
      .map((x) => x.match(/^(?<amount>\d)\s(?<bag>\w+\s*\w+)/))
      .filter((x) => x)
      .map((x) => {
        return {bag: x.groups.bag, amount: parseInt(x.groups.amount)}
      });
  });

  var total = add(bags["shiny gold"]);
