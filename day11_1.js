...
const isOccupied = (coor, seats) => {
    if (coor.x < 0 || coor.y < 0 || coor.x >= seats[0].length || coor.y >= seats.length) {
        return false;
    }
    return seats[coor.y][coor.x] === '#';
}
...
const adjacentSeats = (coor, seats) => {
    ...
    return directions.filter(d => isOccupied(d, seats)).length;
}
