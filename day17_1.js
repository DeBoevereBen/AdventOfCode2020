...
const getNeigbour = (cube, map) => {
    const val = map.get([cube.x, cube.y, cube.z].join`,`);
    return !!val;
}
...
const parse = dataSet => {
    ...
    lines.forEach((l, y) => {
        l.forEach((v, x) => {
            if (v == "#") {
                map.set([x, y, 0].join`,`, true);
            } else {
                map.set([x, y, 0].join`,`, false);
            }
        });
    });
        ...
        for (let z = minz - 1; z <= maxz + 1; z++) {
            for (let y = miny - 1; y <= maxy + 1; y++) {
                for (let x = minx - 1; x <= maxx + 1; x++) {
                    const neighbours = getNeigbours({ x, y, z });
                    const active = neighbours.map(x => getNeigbour(x, map)).filter(x => x).length;
                    if (!!map.get([x, y, z].join`,`)) {
                        if (active == 2 || active == 3) {
                            newMap.set([x, y, z].join`,`, true)
                        } else {
                            newMap.set([x, y, z].join`,`, false)
                        }
                    } else {
                        if (active == 3) {
                            newMap.set([x, y, z].join`,`, true)
