...
const requiredField = ["ecl", "pid", "eyr", "hcl", "byr", "iyr", "hgt"];
let checker = (arr, target) => target.every(v => arr.includes(v));
let validator = (obj) => {

    switch (obj.id) {
        case "byr":
            return obj.value.match(/^\d{4}$/) && obj.value <= 2002 && obj.value >= 1920;
        case "iyr":
            return obj.value.match(/^\d{4}$/) && obj.value <= 2020 && obj.value >= 2010;
        case "eyr":
            return obj.value.match(/^\d{4}$/) && obj.value <= 2030 && obj.value >= 2020;
        case "hgt":
            var match = obj.value.match(/(?<n>\d{2,3})(?<t>in|cm)/);
            if (!match || !match.groups) return false;
            if (match.groups.t == "in")
                return match.groups.n <= 76 && match.groups.n >= 59;
            if (match.groups.t == "cm")
                return match.groups.n <= 193 && match.groups.n >= 150;
            return false;
        case "hcl":
            return !!obj.value.match(/^#[a-f0-9]{6}$/)
        case "ecl":
            return !!obj.value.match(/^amb|blu|brn|gry|grn|hzl|oth$/)
        case "pid":
            return !!obj.value.match(/^[0-9]{9}$/)
        case "cid":
            return true;
        default:
            return false;
...
const parse = () => {
    ...
    return passports.filter(p => {
        const fields = p.split(/\s/g).map(f => {
            return f.split(":")[0];
        });

        if (!checker(fields, requiredField)) {
            return false;
        }

        const data = p.split(/\s/g).map(f => {
            const split = f.split(":");
            return { id: split[0], value: split[1] }
        });

        var isValid = data.every(validator);
